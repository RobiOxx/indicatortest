...
import test.ru.myapplication.BaseDotsIndicator.Type.WORM
...
class WormDotsIndicator @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null,
	...
	private var dotsSelectedColor: Int = 0
	...
	init {
		...
		if (attrs != null) {
			...
			dotsSelectedColor = a.getColor(R.styleable.WormDotsIndicator_selectedDotColor, dotIndicatorColor)
			a.recycle()
		}
	...
	private fun setUpDotIndicator() {
		...
		dotIndicatorView?.apply {
			val dotBackground = background as GradientDrawable
			dotBackground.setColor(dotsSelectedColor)
		...
		val floatPropertyCompat = object : FloatPropertyCompat<View>("DotsWidth") {
			override fun getValue(`object`: View): Float {
				return dotIndicatorView?.layoutParams?.width?.toFloat() ?: 0f
			}
